    ...
    while numel(queue) > 0
        ...
        for curs=1:n
            ...
            if (numinput > 0)
                ...
                if numel(model.tables(curs).truth)==1
%                     curs
                    state(curs)=model.tables(curs).truth;
                    syncstate(curs)=model.tables(curs).truth;
                else
...
%                 end
                ...
                % species has no inputs, check for constant value 
                if numel(model.tables(sp).truth)==1
                   newstate(sp) = model.tables(sp).truth;
                else                    
                    % => keep state
      ...
      %  numinput = numel(model.tables(2).inspecies)
       % syncstate(2)
